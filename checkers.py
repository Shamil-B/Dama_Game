...
class Game:
	...
	def end_turn(self):
		...
		if self.check_for_endgame():
			if self.turn == GREEN:
				self.graphics.draw_message("YELLOW WINS!")
			else:
...
class Graphics:
	def __init__(self):
		...
		self.background = pygame.image.load('/home/shamil/Documents/Computer_Graphics/Pygame-Checkers/resources/board.png')
...
class Board:
	...
	def blind_legal_moves(self, pixel):
		...
		if self.matrix[x][y].occupant != None:
			...
			if self.matrix[x][y].occupant.king == False and self.matrix[x][y].occupant.color == GREEN:
	...
	def king(self, pixel):
		"""
		Takes in (x,y), the coordinates of square to be consideYELLOW for kinging.
		If it meets the criteria, then king() kings the piece in that square and kings it.
		"""
		x = pixel[0]
		y = pixel[1]
		if self.location((x,y)).occupant != None:
			if (self.location((x,y)).occupant.color == GREEN and y == 0) or (self.location((x,y)).occupant.color == YELLOW and y == 7):
